export async function onRequest({ request, params }) {
  ...
  const target = new URL("https://ugqxcarzyjmybyrpdrqx.supabase.co/functions/v1/producao-vip-app/api/users/" + encodeURIComponent(id));
  const headers = new Headers();
  for (const name of ["authorization", "apikey", "content-type", "x-pv-client"]) {
    const value = request.headers.get(name);
    if (value) headers.set(name, value);
  }
  const init = { method: request.method, headers, redirect: "manual", body: await request.arrayBuffer() };
  try {
